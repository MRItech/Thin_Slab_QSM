...
Dx = [Gx_x(1:end-1,:,:); zeros(1,My,Mz)]...
    - [zeros(1,My,Mz); Gx_x(1:end-1,:,:)]; clear Gx_x;

Dy = [Gx_y(:,1:end-1,:), zeros(Mx,1,Mz)]...
    - [zeros(Mx,1,Mz), Gx_y(:,1:end-1,:)]; clear Gx_y;

Dz = cat(3, Gx_z(:,:,1:end-1), zeros(Mx,My,1))...
    - cat(3, zeros(Mx,My,1), Gx_z(:,:,1:end-1)); clear Gx_z

Dx = single(Dx/voxel_size(1));
Dy = single(Dy/voxel_size(2));
Dz = single(Dz/voxel_size(3)); % nash : single
